...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(light_with_sence_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    light_with_sence_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(light_with_sence_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(light_with_sence_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(light_with_sence_M, 0);

  /* Initialize model */
  light_with_sence_initialize();
